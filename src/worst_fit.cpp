...
bool sortMemoryBlockHelper(const MemoryBlock first, const MemoryBlock second) {
...
{
    /*
        Get the largest memory block (sort?)
        see if it can be allocated to the current process
        allocate it
    */
   sort(memory.begin(), memory.end(), sortMemoryBlockHelper);
