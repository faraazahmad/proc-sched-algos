...
int main() {
    ...
    while( finishedQueue.size() != input.size() ) {
        ...
        */
        ...
        /*
            Search for process with longest burst time
            Time complexity: O(n)
            Space Complexity: O(1)

            TODO: Improve time complexity
        */
        int longestBurstTime = readyQueue.at(0).burstTime, longestBurstTimeIndex = 0;
        for (size_t i = 1; i < readyQueue.size(); i++)
        {
            if (readyQueue.at(i).burstTime < longestBurstTime)
            {
                longestBurstTime = readyQueue.at(i).burstTime;
                longestBurstTimeIndex = i;
            }
        }

        // execute the shortest Job
        executeProcess(longestBurstTimeIndex);
    }
